const Achievement = () => {
  return (
    <div className="bg-white md:h-[15rem] flex flex-col items-center justify-center p-5 ">
      <div>
        <h1 className="md:text-5xl text-2xl text-black font-bold font-serif md:mt-[3 rem]"><span className="text-yellow-600">OUR</span> SERVICES</h1>
      </div>
      <div className="md:h-2.5 h-1 bg-black w-20 md:w-40 text-black mt-[1rem]"></div>
      <div className=" mt-8 text-center font-large text-black text-2xl md:text-3xl">
        <h2>Easy and effective way to get you all quick services here!</h2>
      </div>
    </div>
    
  );
};

export default Achievement;
